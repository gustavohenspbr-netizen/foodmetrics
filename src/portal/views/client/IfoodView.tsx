...
import {
  ...
  useIfoodTopItems, useIfoodByNeighborhood, useReviews,
} from "../../lib/api";
  ...
  return (
    <div className="space-y-6">
      ...
      {lSum ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
          <MetricCard label="Faturamento Mês" value={fmt.currencyCompact(summary?.revenue.month ?? 0)} delta={18.4} icon={DollarSign} color="#e01c1c" />
          <MetricCard label="Pedidos Mês" value={fmt.number(summary?.orders.month ?? 0)} delta={12.3} icon={ShoppingBag} color="#ff8732" />
          <MetricCard label="Avaliação" value={(summary?.rating ?? 0).toFixed(1)} deltaLabel="estável" delta={0.1} icon={Star} color="#f59e0b" />
          <MetricCard label="Cancelamentos" value={`${(summary?.cancellations ?? 0).toFixed(1)}%`} delta={-0.3} deltaLabel="melhor" icon={TrendingDown} color="#10b981" />
        </div>
      )}
