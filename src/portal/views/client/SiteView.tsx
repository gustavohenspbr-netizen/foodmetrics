...
import { useMyClient, useSiteAnalytics, useIntegration } from "../../lib/api";

const COLORS = ["#10b981", "#e01c1c", "#ff8732", "#3b82f6"];

export function SiteView() {
  const { data: client } = useMyClient();
  const { data: integration } = useIntegration(client?.id, "ga4");
  const { data: site, loading } = useSiteAnalytics(client?.id);
  const hasData = (site?.totalUsers ?? 0) > 0;
  ...
  return (
    <div className="space-y-6">
      ...
      {!integration && (
        <EmptyState icon={Globe} title="Conecte o GA4 para ver o tráfego do site" />
      )}

      {integration && !hasData && (
        <EmptyState icon={Globe} title="Aguardando primeira sincronização" />
      )}
      ...
      {integration && hasData && (
        <>
          {loading ? (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
              <MetricCard label="Visitantes (30d)" value={fmt.numberCompact(site?.totalUsers ?? 0)} icon={Users} color="#3b82f6" />
              <MetricCard label="Sessões" value={fmt.numberCompact(site?.totalSessions ?? 0)} icon={Globe} color="#10b981" />
              <MetricCard label="Taxa Rejeição" value={`${(site?.avgBounceRate ?? 0).toFixed(0)}%`} icon={MousePointerClick} color="#f59e0b" />
              <MetricCard
                label="Tempo médio"
                value={`${Math.floor((site?.avgSessionSeconds ?? 0) / 60)}m ${Math.round((site?.avgSessionSeconds ?? 0) % 60)}s`}
                icon={Clock} color="#e01c1c"
              />
            </div>
          )}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
            ...
            <ChartCard title="Origem do tráfego" subtitle="Como te encontram" height={280}>
              {sources.length === 0 ? <EmptyState title="Sem dados" /> : (
                <ResponsiveContainer>
                  <PieChart>
