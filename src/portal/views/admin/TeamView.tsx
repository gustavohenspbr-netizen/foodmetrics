...
export function TeamView() {
  ...
  const tabs = [
    { id: "team", label: "Equipe", count: MOCK_TEAM.length },
    { id: "tasks", label: "Tarefas", count: MOCK_TASKS.length },
  ];
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          ...
          <p className="text-[13px] text-slate-500 dark:text-slate-400 font-medium mt-0.5">
            {MOCK_TEAM.length} membros · {MOCK_TASKS.filter((t) => t.status !== "done").length} tarefas em aberto
          </p>
        </div>
      ...
      {view === "team" ? (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
          {MOCK_TEAM.map((m) => {
            const load = (m.tasksOpen / 20) * 100;
            return (
              <Card key={m.id} hoverable>
                <div className="flex items-start justify-between mb-4">
                  <div className="flex items-center gap-3">
                    <Avatar name={m.name} color={m.color} size="lg" status={m.online ? "online" : "offline"} />
                    <div>
                      <p className="text-[15px] font-bold text-slate-900 dark:text-white">{m.name}</p>
                      <p className="text-[12px] text-slate-500 dark:text-slate-400 font-semibold">{m.role}</p>
                    </div>
                  </div>
                ...
                <div className="space-y-3 pt-4 border-t border-slate-100 dark:border-slate-800/60">
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-[13px] font-bold text-slate-900 dark:text-white">{m.clients}</span>
                  </div>
                  <div>
                    ...
                    <ProgressBar
                      value={load}
                      tone={load > 80 ? "danger" : load > 60 ? "warning" : "success"}
                      size="sm"
                    />
                  </div>
                </div>
                ...
                <div className="flex items-center gap-2 mt-4">
                  <Button size="sm" variant="outline" icon={MessageSquare} fullWidth>
                    Mensagem
                  </Button>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-4">
          {TASK_COLUMNS.map((col) => {
            ...
            return (
              <div
                key={col.id}
                className="flex flex-col rounded-2xl bg-slate-50/60 dark:bg-slate-900/30 p-3 min-h-[400px]"
              >
                  ...
                  {colTasks.map((t) => (
                    ...
                    >
                      ...
                      <div className="flex items-center justify-between mt-3 pt-3 border-t border-slate-100 dark:border-slate-800/60">
                        <Avatar name={t.owner} size="xs" color="#ff8732" />
                        <span className="text-[11px] text-slate-500 font-bold">{t.dueDate}</span>
                      </div>
                    </div>
                  ))}
                </div>
